...
ecr-build: ## ECR-step:1 Build your Docker image.
	docker build -t ${APP_IMAGE}:${GIT_COMMIT} ./app/.
...
ecr-tag: ## ECR-step:3 Tag your image so you can push the image to this repository.
	docker tag  ${APP_IMAGE}:${GIT_COMMIT} ${AWS_ACCOUNT}.dkr.ecr.us-east-2.amazonaws.com/${APP_IMAGE}:${GIT_COMMIT}

ecr-push: ## ECR-step:4 Push this image to your newly created AWS repository.
	docker push ${AWS_ACCOUNT}.dkr.ecr.us-east-2.amazonaws.com/${APP_IMAGE}:${GIT_COMMIT}
